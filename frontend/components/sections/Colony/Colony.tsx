...
import DonationModal from "../../utils/DonationModal";
...
export default function ColonySection() {
  const sectionRef = useRef<HTMLElement>(null);
  const [animated,     setAnimated]     = useState(false);
  const [missionData,  setMissionData]  = useState<MissionData>(MOCKUP_DATA);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // keep mockup data on network error
    }
  }, []);
  ...
  const pct        = Math.min(missionData.funding.raised / missionData.funding.goal, 1);
  const pctDisplay = (pct * 100).toFixed(1);

  const missionStats = [
    { label: "Launch Target", value: missionData.launchYear },
    { label: "First Colony",  value: missionData.colonists  },
    { label: "Mission Phase", value: missionData.phase      },
    { label: "Duration",      value: missionData.duration   },
  ];
      ...
      <section ref={sectionRef} id="section-colony" className="colony-section">
        ...
        <div className={`colony-content${animated ? " colony-content--animated" : ""}`}>
          ...
          <div className="colony-left">
            ...
            <button className="colony-cta" onClick={() => setModalOpen(true)}>
              Support the Mission
            </button>
          </div>
          ...
          <div className="colony-right">
            <div className="colony-donors">
              ...
              <div className="colony-donors__list">
                {missionData.topDonors.map((donor) => (
          ...
          <div className="colony-funding">
            <div className="colony-funding__header">
              ...
              <span>
                <span className="colony-funding__raised">{formatUSD(missionData.funding.raised)}</span>
                <span className="colony-funding__of">of</span>
                <span className="colony-funding__goal">{formatUSD(missionData.funding.goal)}</span>
              </span>
            </div>
            ...
            <div className="colony-funding__meta">
              <span>{missionData.funding.supporters.toLocaleString("en-US")} supporters</span>
